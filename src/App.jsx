...
const CREDIT_ROLES = [
  ...
  { canonical: "Writing - review & editing",  patterns: [/review[\s\-]*[&+\/][\s\-]*edit/i, /review[\s\-]+and[\s\-]+edit/i, /writing[\s\-]+review/i] },
];
...
function parseCreditStatement(text, fullNames) {
  ...
  if (format === "role-first") {
    ...
    // Split on periods/semicolons first; if that yields nothing,
    // try splitting on commas that precede an author name + colon
    // e.g. "...writing – review and editing, Shoaib Ahmad: Conceptualization..."
    let sentences = t.split(/(?<=[.;])\s+/).filter(s => s.includes(":"));
    if (sentences.length < 2) {
      // Fallback: split on ", " before what looks like "Name Name:" or "Name Name Name:"
      sentences = t.split(/,\s+(?=[A-Z][a-záéíóúàèìòùäëïöüñ\-]+(?:\s+[A-Z][a-záéíóúàèìòùäëïöüñ\-\.]+)+\s*:)/);
    }
