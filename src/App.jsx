...
function parseCreditStatement(text, fullNames) {
  ...
  if (format === "role-first") {
    ...
    // Narrative format: "Role(s) were carried out by Author1, Author2."
    const sentenceRx = /([^.]+(?:were|was)\s+carried\s+out\s+by\s+[^.]+\.?)/gi;
    const matches = [...t.matchAll(sentenceRx)];
    matches.forEach(m => {
      const sentence = m[1];
      const byIdx = sentence.search(/\bby\b/i);
      if (byIdx === -1) return;
      const rolePart   = sentence.slice(0, byIdx);
      const authorPart = sentence.slice(byIdx + 2);
      // Parse multiple roles from the role part (comma/and separated)
      const roleTokens = rolePart
        .replace(/\band\b/gi, ",")
        .replace(/\bwere?\s+carried\s+out.*/i, "")
        .split(/[,;]+/)
        .map(r => r.trim())
        .filter(Boolean);
      const canonicals = roleTokens.map(r => matchRoleText(r)).filter(Boolean);
      if (!canonicals.length) return;
      tokenizeAuthors(authorPart.replace(/\.\s*$/, "")).forEach(token => {
        const name = matchName(token);
        if (!name) return;
    ...
    // Fallback: split only on periods/semicolons
